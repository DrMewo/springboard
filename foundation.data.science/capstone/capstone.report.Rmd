...
Now, finally we are ready to do run some analytics on the datasets. We will run individual random forests in the two datasets.

```{r running random forest}


rF.tcga <- run.classifier (cbind(tcga$training[["clinical"]], tcga$training[["expression"]]), classifier = "random forest")

predict.rF.tcga <-  predict(rF.tcga, cbind(tcga$testing[["clinical"]], tcga$testing[["expression"]]))

varImpPlot(rF.tcga)

rF.cinsarc <- run.classifier (cbind(cinsarc$training[["clinical"]], cinsarc$training[["expression"]]), classifier = "random forest")
